...
import { FacadeService } from '../../../../../src/app/shared/patterns/facade.service';
...
export class AsideLeftComponent implements OnInit, AfterViewInit {
	...
	ngOnInit() {
		...
		if (localStorage.getItem("userInfo") != null) {
			...
			this.fs.usuarioService.menuPorUser(userInfo.id_user).subscribe((data: any)=> {
				this.menuAsideService.menuList$.next(data);
			});
		}

		//this.menu = this.menuAsideService.menuList$;
	}
	...
	isMenuItemMainIsActive(item): boolean {
		if (item.hijos.length > 0) {
			return this.isMenuRootItemMainIsActive(item);
		}
	...
	isMenuRootItemMainIsActive(item): boolean {
		let result = false;

		for (const subItem of item.hijos) {
			result = this.isMenuItemMainIsActive(subItem);
			if (result) {
				return true;
			}
		}
	...
	getItemMainCssClasses(item) {
		let classes = 'kt-menu__item';

		if (objectPath.get(item, 'hijos')) {
			classes += ' kt-menu__item--submenu';
		}

		if (item.hijos.length == 0 && this.isMenuItemMainIsActive(item)) {
			classes += ' kt-menu__item--active kt-menu__item--here';
		}
	...
	getItemAttrSubmenuMainToggle(item) {
		let toggle = 'hover';
		if (objectPath.get(item, 'toggle') === 'click') {
			toggle = 'click';
		} else if (objectPath.get(item, 'submenu.type') === 'tabs') {
